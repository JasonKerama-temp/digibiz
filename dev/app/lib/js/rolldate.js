    ...
    Rolldate.prototype = {
      ...
      createUI: function createUI() {
        ...
        for (var i = 0; i < len; i++) {
          ...
          //if custom
          if (config.custom.hasOwnProperty('id')) {
            ul += '<div id="' + config.custom.id + '"><ul class="wheel-scroll">';
            var lists = document.querySelectorAll(`.rolldate-custom-list`);
            lists.forEach(function (element, index) {
              ...
              ul += '<li class="wheel-item ' + element.innerHTML + ' ' + activeClass + '" data-index="' + index + '">' + element.innerHTML + '</li>';
            });
        ...
        var _loop = function _loop(_i) {
          if (config.custom && config.custom.hasOwnProperty('id')) {
            var $id = config.custom.id;
          } else {
      ...
      destroy: function destroy(flag) {
        ...
        setTimeout(function () {
          ...
          if (el) {
            //if container
            if (config.container) {
              var cont = document.querySelector(config.container);
              cont.removeChild(el);
            } else {
